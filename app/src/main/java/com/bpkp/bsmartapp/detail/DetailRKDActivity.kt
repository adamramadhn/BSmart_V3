...
import com.bpkp.bsmartapp.databinding.DetailRkdActivityBinding
import kotlinx.android.synthetic.main.detail_surat_tugas.*

class DetailRKDActivity : AppCompatActivity(), View.OnClickListener {
    companion object {
    ...
    private lateinit var binding: DetailRkdActivityBinding
    private lateinit var detailRKDViewModel: DetailRKDViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DetailRkdActivityBinding.inflate(layoutInflater)
    ...
    private fun showDetailRKD(detailSuratTugas: SuratTugasResponse?) {
...
//                tvUserUpdate.text = detailSuratTugas.user_id
                when (userEselon) {
                    "ESELON IV-A", "ESELON IV-B" -> {
                        if (detailSuratTugas.apv_es2 == 1 || detailSuratTugas.apv_es2 == 0 || detailSuratTugas.apv_es1 == 1 || detailSuratTugas.apv_es1 == 0 || detailSuratTugas.apv_es3 == 1 || detailSuratTugas.apv_es3 == 0) {
                            ...
                            if (detailSuratTugas.apv_es4 != 2 && detailSuratTugas.approve_id_user_eselon_4 == DetailRKDActivity.USERID_DETAIL) {
                    ...
                    "ESELON III-A", "ESELON III-B" -> {
                        if (detailSuratTugas.apv_es4 == 0 || detailSuratTugas.apv_es2 == 1 || detailSuratTugas.apv_es2 == 0 || detailSuratTugas.apv_es1 == 1 || detailSuratTugas.apv_es1 == 0) {
                            ...
                            if (detailSuratTugas.apv_es3 != 2 && detailSuratTugas.approve_id_user_eselon_3 == DetailRKDActivity.USERID_DETAIL) {
                                btnCancel.visibility = View.VISIBLE
                                btnTolak.visibility = View.GONE
                                btnSetuju.visibility = View.GONE
                    ...
                    "ESELON II-A", "ESELON II-B" -> {
                        if (detailSuratTugas.apv_es3 == 0 || detailSuratTugas.apv_es4 == 0 || detailSuratTugas.apv_es1 == 1 || detailSuratTugas.apv_es1 == 0) {
                            ...
                            if (detailSuratTugas.apv_es2 != 2 && detailSuratTugas.approve_id_user_eselon_2 == DetailRKDActivity.USERID_DETAIL) {
                                btnCancel.visibility = View.VISIBLE
                                btnTolak.visibility = View.GONE
                                btnSetuju.visibility = View.GONE
                    ...
                    "ESELON I-A", "ESELON I-B" -> {
                        if (detailSuratTugas.apv_es2 == 0 || detailSuratTugas.apv_es4 == 0 || detailSuratTugas.apv_es3 == 0) {
                            ...
                            if (detailSuratTugas.apv_es1 != 2 && detailSuratTugas.approve_id_user_eselon_1 == DetailRKDActivity.USERID_DETAIL) {
                                btnCancel.visibility = View.VISIBLE
                                btnTolak.visibility = View.GONE
                                btnSetuju.visibility = View.GONE
                                if (detailSuratTugas.apv_es2 == 1) {
            ...
            binding.btnSetuju.setOnClickListener {
                val mAlertDialog = AlertDialog.Builder(this@DetailRKDActivity)
                mAlertDialog.setTitle("Perhatian")
                mAlertDialog.setMessage("Apakah Anda yakin menyetujui RKD ini?")
                mAlertDialog.setIcon(R.drawable.ic_warning)

                mAlertDialog.setPositiveButton("Ya") { _, _ ->
                    Handler(mainLooper).postDelayed({
                        detailRKDViewModel.setDetail(userName.toString(), idST)
                        detailRKDViewModel.getDetail().observe(this, {
                            showDetailRKD(it?.get(0))
                            binding.etNote.setText("")
                        })
                    }, 1000)
                    Toast.makeText(this, "Ya", Toast.LENGTH_SHORT).show()
                    try {
                        detailRKDViewModel.suratTugas(
                            userName,
            ...
            binding.btnTolak.setOnClickListener {
                val mAlertDialog = AlertDialog.Builder(this@DetailRKDActivity)
                mAlertDialog.setTitle("Perhatian")
                mAlertDialog.setMessage("Apakah Anda yakin menolak RKD ini?")
                mAlertDialog.setIcon(R.drawable.ic_warning)
                mAlertDialog.setPositiveButton("Ya") { _, _ ->
                    Handler(mainLooper).postDelayed({
                        detailRKDViewModel.setDetail(userName.toString(), idST)
            ...
            binding.btnCancel.setOnClickListener {
                val mAlertDialog = AlertDialog.Builder(this@DetailRKDActivity)
                mAlertDialog.setTitle("Perhatian")
                mAlertDialog.setMessage("Apakah Anda yakin membatalkan tanggapan RKD ini?")
                mAlertDialog.setIcon(R.drawable.ic_warning)
                mAlertDialog.setPositiveButton("Ya") { _, _ ->
                    Handler(mainLooper).postDelayed({
                        detailRKDViewModel.setDetail(userName.toString(), idST)
    ...
    override fun onResume() {
        super.onResume()
        val detailSuratTugas = intent.getParcelableExtra<SuratTugasResponse>(
            DetailRKDActivity.EXTRA_DATA
        )
        userName = intent.getStringExtra(DetailRKDActivity.USERNAME_DETAIL)
        userEselon = intent.getStringExtra(DetailRKDActivity.ESELON_DETAIL)
        detailRKDViewModel = DetailRKDViewModel()
        idST = detailSuratTugas!!.id_st
        showDetailRKD(detailSuratTugas)
    }
